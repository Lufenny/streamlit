...
selected_years = st.sidebar.slider("Select Year Range", min_value=int(df['Year'].min()), max_value=int(df['Year'].max()), value=(2016, 2022))
selected_countries = st.sidebar.multiselect("Select Countries", options=sorted(df['Country'].unique()), default=["India", "Brazil", "Indonesia"])
...
fig_line = px.line(line_df, x="Year", y="Amount", color="Country", markers=True,
...
fig_bar = px.bar(bar_df, x='Country', y='Amount', color='Country', title=f"Total {selected_metric} (2016–2022)")
st.plotly_chart(fig_bar, use_container_width=True)

# --- Optional: Choropleth Map ---
if selected_metric == "Production":
    st.subheader("🌍 Average Production Map")
    avg_df = df[(df['Metric'] == "Production") & (df['Year'] >= selected_years[0]) & (df['Year'] <= selected_years[1])]
    avg_prod = avg_df.groupby('Country')['Amount'].mean().reset_index()
    fig_map = px.choropleth(avg_prod, locations="Country", locationmode="country names",
